...
def get_history_retriever():
    ...
    contextualize_q_prompt = ChatPromptTemplate.from_messages(
        [
            ...
            MessagesPlaceholder("chat_history"), # 대화 기록을 여기 넣음
            ("human", "{input}"),
        ]
    )
...
def get_rag_chain():
    ...
    qa_prompt = ChatPromptTemplate.from_messages(
        [
            ("system", system_prompt), #대화 내용 기반 시스템 프롬프트가 작동되도록
            few_shot_prompt,
            MessagesPlaceholder("chat_history"), #이전 대화 내용
            ("human", "{input}"), 
        ]
    )
...
def get_ai_message(user_message):
    ...
    ai_message = tax_chain.stream( #invoke 대신  stream으로 하면 iterator로 바뀜
